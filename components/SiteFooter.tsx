...
export function SiteFooter() {
  return (
    <footer className="border-t border-[var(--border)] bg-[var(--background)]">
      <div className="max-w-5xl mx-auto px-5 md:px-10 py-10 flex flex-col items-center gap-6">
        <div className="flex flex-wrap items-center justify-center gap-2">
          {profileLinks.map((link) => (
            <a
              ...
              className="group inline-flex items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--panel)]/60 px-3.5 py-1.5 text-xs text-[var(--muted)] hover:border-[var(--accent)]/50 hover:bg-[var(--panel)] hover:text-[var(--foreground)] transition-colors"
            >
              <span className="grid h-4 w-4 place-items-center text-[var(--muted)] group-hover:text-[var(--accent-2)] transition-colors">
                {link.icon}
              </span>
...
function LeetCodeIcon() {
  return (
    <span className="font-mono text-[10px] font-semibold leading-none" aria-hidden>
      LC
    </span>
  );
}
